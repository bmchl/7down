...
# Gestion des clés API et des informations sensibles

Pour des raisons de sécurité, les fichiers contenant des clés API, des identifiants et d'autres informations sensibles ne sont pas inclus dans le dépôt Git. Voici comment configurer ces fichiers après avoir cloné le projet :

## Firebase Service Account

1. Pour le serveur, vous devez créer un fichier `server/app/utils/fbkey.json` à partir du fichier template `server/app/utils/fbkey.template.json`.
2. Remplissez-le avec vos informations de compte de service Firebase que vous pouvez obtenir à partir de la console Firebase.

## Configuration Firebase

1. Pour le client desktop, créez un fichier `desktop-client/firebase/firebaseConfig.ts` à partir du template `desktop-client/firebase/firebaseConfig.template.ts`.
