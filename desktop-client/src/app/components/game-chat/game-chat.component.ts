...
import { ParticipantDialogComponent } from '@app/dialogs/participant-dialog/participant-dialog.component';
...
import { equalTo, get, getDatabase, off, onValue, orderByChild, push, query, ref, remove, set, update } from 'firebase/database';
...
@Component({
    selector: 'app-game-chat-component',
...
export class GameChatComponent implements OnInit {
    ...
    messages: { sender: string; username: string; text: string; time: string; avatarUrl?: any }[] = [];
    ...
    showParticipants(roomId: string): void {
                ...
                get(roomRef)
                    .then((snapshot) => {
                        if (snapshot.exists()) {
                            ...
                            const isAdmin = roomData.admin === this.uid;
                            this.dialog
    ...
    // Method to fetch participants data from the database
        ...
        onValue(messagesRef, async (snapshot) => {
            ...
            if (messages) {
                ...
                for (const key in messages) {
                    ...
                    if (!blocked) {
                        const avatarUrl = await this.fetchAvatarUrl(senderUid);
                        messages[key].avatarUrl = avatarUrl;
                        filteredMessages.push({
                            sender: messages[key].sender,
                            username: messages[key].username,
                            text: messages[key].message,
                            time: messages[key].time,
                            avatarUrl: messages[key].avatarUrl,
                        });
                    }
                }
    ...
    async fetchAvatarUrl(uid: string): Promise<string | null> {
        const db = getDatabase();
    ...
    detachChat(): void {
        const chatUrl = `${window.location.origin}/standalone-chat?standalone=true`;
    ...
    // sanitizeMessage(message: string): void {
    ...
    fetchRooms(): void {
        ...
        onValue(roomsRef, (snapshot) => {
            ...
            for (const roomId in roomsData) {
                ...
                // Skip adding Global Chat again as it's already added
                if (roomId !== 'global') {
                    if (isParticipant) {
                        userRooms.push({
                            id: roomId,
    ...
    tabChanged(tabIndex: number) {
        if (tabIndex === 1) {
            // Assuming "Game Room" is the second tab
            this.fetchRooms();
        }
    }

    selectGameRoom(roomId: string) {
        this.currentRoom = roomId;
        // Add other actions, like fetching messages for the room
        this.fetchMessages(roomId);
        // Add navigation or UI updates to enter the selected room's chat
    }
