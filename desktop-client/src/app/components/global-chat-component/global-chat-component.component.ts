...
@Component({
    selector: 'app-global-chat-component',
    templateUrl: './global-chat-component.component.html',
    styleUrls: ['./global-chat-component.component.scss'],
})
    ...
    async ngOnInit() {
        ...
        this.socketService.on('global-message', (data: { sender: string; username: string; message: string }) => {
            this.messages.push({
                ...
                text: /*this.sanitizeMessage(data.message)*/ data.message,
        ...
        this.fetchParticipants(roomId)
            .then((participants) => {
                this.dialog.open(ParticipantDialogComponent, {
                    width: '250px',
                    data: { roomId: name, participants: participants },
                });
            })
            .catch((error) => {
        ...
        onValue(roomsRef, (snapshot) => {
            ...
            for (const roomId in roomsData) {
                ...
                if (isParticipant) {
                    userRooms.push({
                        id: roomId,
                        name: ' Match Room', // Append "(Game Chat)" if it's a game room
